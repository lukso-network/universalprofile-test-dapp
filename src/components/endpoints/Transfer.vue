...
const transfer = async () => {
  ...
  switch (tokenType.value) {
    case ContractStandard.LSP7:
      ...
      try {
        await myToken.value.methods
          .transfer(erc725AccountAddress, transferReceiver.value, toWei(transferAmount.value.toString()), transferForce.value, '0x')
    ...
    case ContractStandard.LSP8:
      ...
      try {
        await myToken.value.methods
          .transfer(erc725AccountAddress, transferReceiver.value, tokenId.value, transferForce.value, '0x')
          .send({ from: erc725AccountAddress })
          .on('receipt', (receipt: any) => {
            console.log(receipt)
          })
  ...
  <div class="tile is-4 is-parent">
    <div class="tile is-child box">
      ...
      <div class="field">
        ...
        <LSPSelect :address="transferToken" :show-types="[LSPType.LSP7DigitalAsset, LSPType.LSP8IdentifiableDigitalAsset, LSPType.ERC20, LSPType.ERC777]" @option-selected="handleTokenSelected" />
        <div class="control">
          <input v-model="transferToken" class="input is-family-code" type="text" data-testid="transfer-address" />
        </div>
      </div>
      <div class="field">
        <label class="label">Transfer address</label>
        <LSPSelect :show-accounts="true" :address="transferReceiver" @option-selected="handleTokenReceiverSelected" />
        <div class="control">
          <input v-model="transferReceiver" class="input is-family-code" type="text" data-testid="transfer-address" />
        </div>
      </div>
      <div v-if="tokenType === ContractStandard.LSP7" class="field">
        ...
        <div class="control columns">
          <div class="column is-one-third">
            <input v-model="transferAmount" class="input" type="number" placeholder="0" />
          </div>
        </div>
      </div>
      <div v-else class="field">
        ...
        <div class="control">
          <input v-model="tokenId" class="input" type="text" data-testid="transfer-address" placeholder="0xbb204573da1a42ab80f38995444b17124110b946ba189157ffcc7ba2b3375bf8" @blur="handleBlurTokenId" />
        </div>
      </div>
      <div class="field">
        <button class="button is-primary is-rounded mb-3 mr-3" data-testid="transfer" @click="transfer">Transfer</button>
      </div>
      <div class="field">
