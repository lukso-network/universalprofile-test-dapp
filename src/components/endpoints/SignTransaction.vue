...
import { UnsignedTransaction, RLP, joinSignature, keccak256, serializeTransaction } from 'ethers/lib/utils'
...
const onPermissionsValidation = async () => {
  ...
  const erc725Account = contract(UniversalProfile.abi as any, getState('address'), {
    gasPrice: DEFAULT_GAS_PRICE,
    gas: Number.MAX_VALUE,
  })

  const permissionsResponse = await erc725Account.methods.getData(ERC725YDataKeys.LSP6['AddressPermissions:Permissions'] + stripHexPrefix(signerAddress)).call()

  const decodedPermissions: Permissions = ERC725.decodePermissions(permissionsResponse)

  if (!decodedPermissions.SIGN) {
    throw new Error(`Signer doesn't have SIGN permission. Transaction will fail.`)
  }
...
const onSignatureValidation = async () => {
  ...
  try {
    ...
    if (!decodedTransaction || decodedTransaction.length < 3) {
      throw new Error(`Failed to decode transaction. RLP.decode response is: ${JSON.stringify(decodedTransaction)}`)
    }
    ...
    magicValue.value = (await erc725Account.methods.isValidSignature(transactionHashedForSignature.value, signature).call()) as string
...
const selectMethod = (e: Event) => {
  ...
  const item: MethodSelect = value >= methods.length ? items.items[value - methods.length] : methods[value]
...
const handleAdd = (e: Event) => {
  ...
  if (name) {
    items.items.push(JSON.parse(JSON.stringify({ ...method.item, label: name, inputs: params.items })))
    saveItems()
  }
}
  ...
  <div class="tile is-4 is-parent">
    <div class="tile is-child box">
      ...
      <div class="field mt-5">
        <button :class="`button is-primary is-rounded mb-3 ${isPending ? 'is-loading' : ''}`" data-testid="sign" @click="onSign">Sign transaction</button>
      </div>
      <div class="field">
      ...
      <div class="field">
        <button class="button is-primary is-rounded mb-3" :disabled="recovery ? undefined : true" data-testid="validate-permission" @click="onPermissionsValidation">Validate permissions</button>
      </div>
      <div class="field">
      ...
      <div class="field">Test <code>eth_signTransaction</code> RPC call [<a href="https://ethereum.org/en/developers/docs/apis/json-rpc/#eth_signtransaction">documentation</a>].</div>
      <div class="field">
        How to
