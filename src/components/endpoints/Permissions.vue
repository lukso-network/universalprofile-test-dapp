...
const ALL_PERMISSIONS_WITH_DELEGATECALL = padLeft(numberToHex((hexToNumber(ALL_PERMISSIONS, false) as number) + (hexToNumber(PERMISSIONS.DELEGATECALL, false) as number) + (hexToNumber(PERMISSIONS.SUPER_DELEGATECALL, false) as number)), 64)

const setPermissions = async () => {
  clearNotification()
  const erc725AccountAddress = getState('address')

  const dataKeysToSet = [ERC725YDataKeys.LSP6['AddressPermissions:Permissions'] + grantPermissionAddress.value.slice(2)]
  ...
  const [{ value: controllerList }, { value: currentPermissions }] = controllersDataResult
...
const allPermissionsToggle = () => {
  if (allPermissionsSelected.value) {
    ...
    selectedPermissions.value = decodePermissions(ALL_PERMISSIONS_WITH_DELEGATECALL)
  }
}

const allPermissionsSelected = computed(() => {
  return encodePermissions(selectedPermissions.value) === ALL_PERMISSIONS_WITH_DELEGATECALL
})
</script>
  ...
  <div class="tile is-4 is-parent">
    <div class="tile is-child box">
      ...
      <div class="field">
        ...
        <div class="control">
          <input v-model="grantPermissionAddress" class="input is-family-code" type="text" />
        </div>
      </div>
      <div class="field">
        <div v-for="(value, key, index) in permissions" :key="index" class="mb-2">
          <label class="checkbox" :data-testid="key">
            <input v-model="selectedPermissions[key]" type="checkbox" :value="value" />
            {{ key }}
          </label>
        </div>
      </div>
      <div class="field">
        <div class="mb-2">
          <label class="checkbox">
            <input :checked="allPermissionsSelected" type="checkbox" @click.stop="allPermissionsToggle" />
            Select all
          </label>
        </div>
      </div>
      <div class="field">
        <div v-if="getState('isConnected')" class="notification is-info is-light mt-5" data-testid="info">
          <p class="">
            Hex:
          ...
          <p class="">
            Decimal:
            <b>{{ Web3Utils.toDecimal(encodePermissions(selectedPermissions)) }}</b>
          </p>
          <p data-testid="chain">
            Bytes:
      ...
      <div class="field">
        <button :class="`button is-primary is-rounded mb-3 ${isPending ? 'is-loading' : ''}`" data-testid="setPermissions" @click="setPermissions">Set permissions</button>
      </div>

      <div class="field"><a href="https://docs.lukso.tech/guides/key-manager/give-permissions">Key Manager permissions tutorial</a>.</div>
