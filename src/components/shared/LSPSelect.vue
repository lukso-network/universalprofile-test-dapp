...
const { sampleUP, sampleEoA, sampleSC, erc20TokenWithEip165, erc20TokenWithoutEip165, erc777TokenWithEip165, erc777TokenWithoutEip165, erc721TokenWithEip165, errorContract } = getSelectedNetworkConfig()
...
const findItem = (value: string): TokenInfo | undefined => {
  const items = Object.entries(data.value)
    .map(([, items]: [string, TokenInfo[]]) => {
      const found = items.find(({ address, type }) => address?.toLowerCase() === value.toLowerCase() || type === value)
      return found
    })
...
const selectFirst = () => {
  if (props.address) {
    const corresponding: undefined | TokenInfo = Object.entries(data.value).reduce((first: undefined | TokenInfo, [, items]) => {
      if (first) {
        return first
      }
    ...
    const first: undefined | TokenInfo = Object.entries(data.value).reduce((first: undefined | TokenInfo, [, items]) => {
      if (first) {
        return first
      }
...
watch(
  () => {
    return [getState('lsp7'), getState('lsp8'), getState('address'), props.address]
  },
  ([lsp7, lsp8, address]) => {
    const newData: { [key: string]: TokenInfo[] } = {}
    let items: TokenInfo[]
    if (props.showUp || props.showAccounts || props.showTypes?.includes(LSPType.UP) || props.showTypes == null) {
    ...
    if (props.showAny || props.showAccounts || props.showTypes?.includes(LSPType.SC) || props.showTypes == null) {
    ...
    if (props.showAny || props.showAccounts || props.showTypes?.includes(LSPType.EoA) || props.showTypes == null) {
    ...
    if (props.showAny || props.showTypes?.includes(LSPType.ERC20) || props.showTypes == null) {
    ...
    if (props.showAny || props.showTypes?.includes(LSPType.ERC777) || props.showTypes == null) {
    ...
    if (props.showAny || props.showTypes?.includes(LSPType.ERC721) || props.showTypes == null) {
    ...
    if (props.showAny || props.showTypes?.includes(LSPType.ERROR_TEST) || props.showTypes == null) {
    ...
    if (props.showTypes ? props.showTypes.includes(LSPType.LSP7DigitalAsset) : true) {
      children = lsp7
    ...
    if (props.showTypes ? props.showTypes.includes(LSPType.LSP8IdentifiableDigitalAsset) : true) {
      children = lsp8
      if (children.length > 0 || props.showLspType) {
  ...
  <div class="field">
    <div class="select is-fullwidth">
      <select v-model="selected" @change="handleChange">
        ...
        <optgroup v-for="(items, label) of data" :key="label" :label="`${label}`">
          <option v-for="option in items" :key="option.address" :value="option.address || option.type">
            {{ option.label }}
          </option>
        </optgroup>
      </select>
