...
interface UPClientChannel {
	 ...
	 */
	listeners<T extends EventEmitter.EventNames<UPClientChannelEvents>>(
		event: T,
	): Array<EventEmitter.EventListener<UPClientChannelEvents, T>>;
	 ...
	 */
	emit<T extends EventEmitter.EventNames<UPClientChannelEvents>>(
		event: T,
		...args: EventEmitter.EventArgs<UPClientChannelEvents, T>
	): boolean;

	/**
	 * Add a listener for a given event.
	 */
	on<T extends EventEmitter.EventNames<UPClientChannelEvents>>(
		event: T,
		fn: EventEmitter.EventListener<UPClientChannelEvents, T>,
		context?: any,
	): this;
	addListener<T extends EventEmitter.EventNames<UPClientChannelEvents>>(
		event: T,
		fn: EventEmitter.EventListener<UPClientChannelEvents, T>,
		context?: any,
	): this;
	 ...
	 */
	once<T extends EventEmitter.EventNames<UPClientChannelEvents>>(
		event: T,
		fn: EventEmitter.EventListener<UPClientChannelEvents, T>,
		context?: any,
	): this;
	 ...
	 */
	removeListener<T extends EventEmitter.EventNames<UPClientChannelEvents>>(
		event: T,
		fn?: EventEmitter.EventListener<UPClientChannelEvents, T>,
		context?: any,
		once?: boolean,
	): this;
	off<T extends EventEmitter.EventNames<UPClientChannelEvents>>(
		event: T,
		fn?: EventEmitter.EventListener<UPClientChannelEvents, T>,
		context?: any,
		once?: boolean,
	 ...
	 */
	removeAllListeners(
		event?: EventEmitter.EventNames<UPClientChannelEvents>,
	): this;
...
class _UPClientChannel
	extends EventEmitter3<UPClientChannelEvents>
	implements UPClientChannel
{
	#accounts: (`0x${string}` | "")[] = [];
	#chainId = 0;
	#rpcUrls: string[] = [];
	...
	public async allowAccounts(
		...
		const primaryChanged =
			this.#accounts[0] !== primary || this.#getter() !== enabled;
		const pageChanged = this.#accounts
			.slice(1)
			.some((value, index) => value !== page[index]);
		this.#setter(enabled);
		if (primaryChanged || pageChanged) {
			...
			await this.send("accountsChanged", [
				this.#getter() ? primary : "",
				...this.#accounts.slice(1),
			]);
			if (primaryChanged) {
				this.emit(
...
interface UPProviderConnectorEvents {
	channelCreated: (
		id: HTMLIFrameElement | Window | string,
...
interface UPProviderConnector {
	 ...
	 * Return the listeners registered for a given event.
	 */
	listeners<T extends EventEmitter.EventNames<UPProviderConnectorEvents>>(
		event: T,
	): Array<EventEmitter.EventListener<UPProviderConnectorEvents, T>>;

	/**
	 * Return the number of listeners listening to a given event.
	 */
	listenerCount(
		event: EventEmitter.EventNames<UPProviderConnectorEvents>,
	): number;

	/**
	 * Calls each of the listeners registered for a given event.
	 */
	emit<T extends EventEmitter.EventNames<UPProviderConnectorEvents>>(
		event: T,
		...args: EventEmitter.EventArgs<UPProviderConnectorEvents, T>
	): boolean;

	/**
	 * Add a listener for a given event.
	 */
	on<T extends EventEmitter.EventNames<UPProviderConnectorEvents>>(
		event: T,
		fn: EventEmitter.EventListener<UPProviderConnectorEvents, T>,
		context?: any,
	): this;
	addListener<T extends EventEmitter.EventNames<UPProviderConnectorEvents>>(
		event: T,
		fn: EventEmitter.EventListener<UPProviderConnectorEvents, T>,
		context?: any,
	): this;
	 ...
	 * Add a one-time listener for a given event.
	 */
	once<T extends EventEmitter.EventNames<UPProviderConnectorEvents>>(
		event: T,
		fn: EventEmitter.EventListener<UPProviderConnectorEvents, T>,
		context?: any,
	): this;
	 ...
	 * Remove the listeners of a given event.
	 */
	removeListener<T extends EventEmitter.EventNames<UPProviderConnectorEvents>>(
		event: T,
		fn?: EventEmitter.EventListener<UPProviderConnectorEvents, T>,
		context?: any,
		once?: boolean,
	): this;
	off<T extends EventEmitter.EventNames<UPProviderConnectorEvents>>(
		event: T,
		fn?: EventEmitter.EventListener<UPProviderConnectorEvents, T>,
		context?: any,
		once?: boolean,
...
class _UPProviderConnector extends EventEmitter3<UPProviderConnectorEvents> {
	...
	async injectAddresses(...page: (`0x${string}` | "")[]) {
		const changed = this.#options.accounts
			.slice(1)
			.some((value, index) => page?.[index] !== value);
		if (changed) {
			this.#options.accounts = [this.#options.primary, ...page];
			for (const item of this.channels.values()) {
				await item.allowAccounts(
					item.enabled,
					[this.#options.primary, ...this.#options.accounts.slice(1)],
		...
		this.#options.promise = new Promise<void>((resolve, reject) => {
			(async () => {
				try {
					...
					if (
						newRpcUrls.some(
							(url, index) => url !== this.#options.rpcUrls[index],
						)
					) {
		...
		if (event.data === "upProvider:hasProvider") {
				...
				switch (method) {
					...
					case "accounts":
						serverLog("short circuit response", request, [
							options.primary,
							...channel_.accounts.slice(1),
						]);
						channel_.emit("requestAccounts", [
							enabled ? options.primary : "",
							...channel_.accounts.slice(1),
						]);
						return {
							...request,
			...
			const channelHandler = (event: MessageEvent) => {
				if (event.data.type === "upProvider:windowInitialized") {
					...
					globalUPProvider?.emit(
						"channelCreated",
						channel_.element || channel_.window || null,
						channel_,
					);
...
export {
	type UPClientChannel,
	type UPClientChannelEvents,
	type UPProviderConnector,
	type UPProviderConnectorEvents,
